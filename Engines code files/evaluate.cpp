///Evaluation function used for chess engines.
///All weights set to 1. Replace them with the weights you generate.
...
namespace {
  ...
  ///WEIGHTS - put weights generated from either method here.
  ...
  Value Evaluation::value() {
    ...
    //Material
    	...
    	+ bishopPairWeight*bishopPair(pos)
        //King Safety
        + shelterWeight * (kingShelter(pos,WHITE) - kingShelter(pos,BLACK))
        + pawnStormWeight * (pawnStorm(pos,WHITE) - pawnStorm(pos,BLACK))
        + kingEscapeWeight * (kingEscapeRoutes(pos,WHITE) - kingEscapeRoutes(pos,BLACK))
        + kingTropismWeight * (kingTropism(pos,WHITE) - kingTropism(pos,BLACK))
        //Pawn Structure
        + doubledPWeight * (doubledPawns(pos,WHITE) - doubledPawns(pos,BLACK))
        + isolatedPWeight * (isolatedPawns(pos,WHITE) - isolatedPawns(pos,BLACK))
        + PPWeight * (passedPawns(pos,WHITE) - passedPawns(pos,BLACK))
        + rooksBehindWeight * (rooksBehindPP(pos,WHITE) - rooksBehindPP(pos,BLACK))
        + backwardPWeight * (backwardPawns(pos,WHITE) - backwardPawns(pos,BLACK))
        + chainsWeight * (pawnChains(pos,WHITE) - pawnChains(pos,BLACK))
        + pawnMobWeight * (pawnMobility(pos,WHITE) - pawnMobility(pos,BLACK))
        + phalanxWeight * (pawnPhalanx(pos,WHITE) - pawnPhalanx(pos,BLACK))
        + outpostsWeight * (pawnOutposts(pos,WHITE) - pawnOutposts(pos,BLACK))
        //Piece Mobility
        + knightMobWeight * (knightMobility(pos,WHITE) - knightMobility(pos,BLACK))
        + bishopMobWeight * (bishopMobility(pos,WHITE) - bishopMobility(pos,BLACK))
        + rookMobWeight * (rookMobility(pos,WHITE) - rookMobility(pos,BLACK))
        + queenMobWeight * (queenMobility(pos,WHITE) - queenMobility(pos,BLACK))
        + ccWeight * (centralControl(pos,WHITE) - centralControl(pos,BLACK))
        + rookFileWeight * (rookOnFile(pos,WHITE) - rookOnFile(pos,BLACK))
        + queenFileWeight * (queenOnFile(pos,WHITE) - queenOnFile(pos,BLACK))
        //Trapped Pieces
        + trapBishWeight * trappedBishop(pos)
        + trapRookWeight * (rookTrappedByKing(pos,WHITE) - rookTrappedByKing(pos,BLACK))
        //Tactics
        + pinsWeight * (havePins(pos,WHITE) - havePins(pos,BLACK))
        + forksWeight * (haveForks(pos,WHITE) - haveForks(pos,BLACK))
        + skewersWeight * (haveSkewers(pos,WHITE) - haveSkewers(pos,BLACK))
        + discAttWeight * (discoveredAttacks(pos,WHITE) - discoveredAttacks(pos,BLACK))
        + doubAttWeight * (doubleAttacks(pos,WHITE) - doubleAttacks(pos,BLACK))
        + overloadWeight * (haveOverloads(pos,WHITE) - haveOverloads(pos,BLACK))
        + hangingWeight * (hangingPieces(pos,WHITE) - hangingPieces(pos,BLACK))
        //Endgame
        + egkingActWeight* (egkingActivity(pos,WHITE) - egkingActivity(pos,BLACK))
        //Piece Co-ordination
        + rookCoordWeight * (rooksCoordination(pos,WHITE) - rooksCoordination(pos,BLACK))
        + otherCoordWeight * (othersCoordination(pos,WHITE) - othersCoordination(pos,BLACK))
        + kingCoordWeight * (kingsCoordination(pos,WHITE) - kingsCoordination(pos,BLACK))
        + fiancettoWeight * (fianchettoBishops(pos,WHITE) - fianchettoBishops(pos,BLACK))
        //Development
        + developWeight * (developPieces(pos,WHITE) - developPieces(pos,BLACK));
        vv*=PawnValueEg;
        vv*=0.01;
