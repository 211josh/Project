...
///Variant which performs depth 2 search.
...
const int no_positions = 10; //no. positions in fitness evaluation. randomly selected each generation out of 4850 possible positions.
const int no_generations = 3; //max no. iterations of algorithm.
...
void accessDB(){
    ...
    const char *sql = "SELECT id, fen, depth2eval FROM evaluations LIMIT 4850";
...
//            eval_difference[i] += fabs(eval_temp - evalDB4850[randomPositions[n]]); //overall difference in centipawns across positions.
  ...
  gp << "set ylabel 'Average evaluation difference across puzzles (centipawns)'\n";
  gp << "set title 'Evaluation difference of best sample per generation'\n";
  gp << "set yrange [0:*]\n";
